...
La première ligne ajoutée affecte à la propriété \texttt{mFirstName} la valeur \texttt{"Joe"}. La propriété \texttt{mFirstName} ait été déclarée du type \eb!String! dans le texte source \emph{Easy Binding}, celle-ci est représentée dans le code Swift engendré par une instance de la classe \texttt{EBStoredProperty\_String}. Celle-ci possède une méthode \texttt{setProp} qui permet de modifier la valeur de la propriété par programme.
