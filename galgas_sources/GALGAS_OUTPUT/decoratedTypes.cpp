...
void routine_checkDefaultValueType (const GALGAS_easyBindingTypeList constinArgument_inDefaultValueCorrespondingAcceptableTypes,
  ...
  cEnumerator_easyBindingTypeList enumerator_13580 (constinArgument_inDefaultValueCorrespondingAcceptableTypes, kEnumeration_up) ;
  //--- complex foreach instruction
  bool bool_0 = var_found.operator_not (SOURCE_FILE_AT_LINE (374)).isValidAndTrue () ;
  if (enumerator_13580.hasCurrentObject () && bool_0) {
    while (enumerator_13580.hasCurrentObject () && bool_0) {
      //-------- @assignmentInstructionForGeneration
      var_found = GALGAS_bool (kIsEqual, constinArgument_inAttributeActualType.objectCompare (enumerator_13580.current_mType (HERE))) ;
      enumerator_13580.gotoNextObject () ;
      if (enumerator_13580.hasCurrentObject ()) {
        bool_0 = var_found.operator_not (SOURCE_FILE_AT_LINE (374)).isValidAndTrue () ;
      }
    }
  }
  //-------- @ifInstructionForGeneration
  const enumGalgasBool test_1 = var_found.operator_not (SOURCE_FILE_AT_LINE (377)).boolEnum () ;
  if (kBoolTrue == test_1) {
    //-------- @errorInstructionForGeneration
    GALGAS_location location_2 (constinArgument_inAttributeName.reader_location (HERE)) ; // Implicit use of 'location' reader
    inCompiler->emitSemanticError (location_2, GALGAS_string ("the default value is invalue for this attribute") COMMA_SOURCE_FILE_AT_LINE (378)) ;
  }
}
