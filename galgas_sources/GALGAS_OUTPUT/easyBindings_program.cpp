...
void routine_programRule_5F__30_ (const GALGAS_lstring constinArgument_inSourceFile,
  ...
  const enumGalgasBool test_0 = GALGAS_bool (kIsStrictSup, var_preferencesMap.reader_count (SOURCE_FILE_AT_LINE (61)).objectCompare (GALGAS_uint ((PMUInt32) 0U))).boolEnum () ;
  if (kBoolTrue == test_0) {
    //-------- @plusEqualnstructionForGeneration
    var_sources.addAssign_operation (GALGAS_string ("PMAbstractPreferences") COMMA_SOURCE_FILE_AT_LINE (62)) ;
  }
  //-------- @ifInstructionForGeneration
  const enumGalgasBool test_1 = GALGAS_bool (kIsStrictSup, var_documentMap.reader_count (SOURCE_FILE_AT_LINE (64)).objectCompare (GALGAS_uint ((PMUInt32) 0U))).boolEnum () ;
  if (kBoolTrue == test_1) {
    //-------- @plusEqualnstructionForGeneration
    var_sources.addAssign_operation (GALGAS_string ("PMAbstractPersistentDocument") COMMA_SOURCE_FILE_AT_LINE (65)) ;
  }
  //-------- @ifInstructionForGeneration
  const enumGalgasBool test_2 = GALGAS_bool (kIsStrictSup, var_entityMap.reader_count (SOURCE_FILE_AT_LINE (67)).objectCompare (GALGAS_uint ((PMUInt32) 0U))).boolEnum () ;
  if (kBoolTrue == test_2) {
    //-------- @plusEqualnstructionForGeneration
    var_sources.addAssign_operation (GALGAS_string ("PMAbstractManagedObject") COMMA_SOURCE_FILE_AT_LINE (68)) ;
  }
  //-------- @localVariableOrConstantDeclarationWithSourceExpressionForGeneration
  ...
  cEnumerator_stringlist enumerator_2666 (var_fw.reader_allTextFilePathes (SOURCE_FILE_AT_LINE (71)), kEnumeration_up) ;
  //--- simple foreach instruction
  while (enumerator_2666.hasCurrentObject ()) {
    //-------- @localVariableOrConstantDeclarationWithSourceExpressionForGeneration
    GALGAS_string var_sourceName = enumerator_2666.current_mValue (HERE).reader_lastPathComponent (SOURCE_FILE_AT_LINE (72)) ;
    //-------- @ifInstructionForGeneration
    const enumGalgasBool test_3 = var_sources.reader_hasKey (var_sourceName.reader_stringByDeletingPathExtension (SOURCE_FILE_AT_LINE (73)) COMMA_SOURCE_FILE_AT_LINE (73)).boolEnum () ;
    if (kBoolTrue == test_3) {
      { //-------- @typeMethodInstructionForGeneration
      GALGAS_string::class_method_generateFile (var_sourceDir.operator_concat (enumerator_2666.current_mValue (HERE).reader_stringByDeletingLastPathComponent (SOURCE_FILE_AT_LINE (75)) COMMA_SOURCE_FILE_AT_LINE (75)), var_sourceName, var_fw.reader_textFileContentsAtPath (enumerator_2666.current_mValue (HERE), inCompiler COMMA_SOURCE_FILE_AT_LINE (77)), inCompiler COMMA_SOURCE_FILE_AT_LINE (74)) ;
      } //-------- @routineCallInstructionForGeneration
    }
