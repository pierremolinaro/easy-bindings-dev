...
void routine_programRule_5F__30_ (const GALGAS_lstring constinArgument_inSourceFile,
  ...
  const enumGalgasBool test_0 = GALGAS_bool (kIsStrictSup, var_documentMap.reader_count (SOURCE_FILE_AT_LINE (83)).objectCompare (GALGAS_uint ((PMUInt32) 0U))).boolEnum () ;
  if (kBoolTrue == test_0) {
    //-------- @plusEqualnstructionForGeneration
    var_sources.addAssign_operation (GALGAS_string ("PMAbstractPersistentDocument") COMMA_SOURCE_FILE_AT_LINE (84)) ;
  }
  //-------- @ifInstructionForGeneration
  const enumGalgasBool test_1 = GALGAS_bool (kIsStrictSup, var_entityMap.reader_count (SOURCE_FILE_AT_LINE (86)).objectCompare (GALGAS_uint ((PMUInt32) 0U))).boolEnum () ;
  if (kBoolTrue == test_1) {
    //-------- @plusEqualnstructionForGeneration
    var_sources.addAssign_operation (GALGAS_string ("PMAbstractManagedObject") COMMA_SOURCE_FILE_AT_LINE (87)) ;
  }
  //-------- @ifInstructionForGeneration
  const enumGalgasBool test_2 = GALGAS_bool (kIsStrictSup, var_classMap.reader_count (SOURCE_FILE_AT_LINE (89)).objectCompare (GALGAS_uint ((PMUInt32) 0U))).boolEnum () ;
  if (kBoolTrue == test_2) {
    //-------- @plusEqualnstructionForGeneration
  ...
  cEnumerator_stringlist enumerator_3274 (var_fw.reader_allTextFilePathes (SOURCE_FILE_AT_LINE (93)), kEnumeration_up) ;
  //--- simple foreach instruction
  while (enumerator_3274.hasCurrentObject ()) {
    //-------- @localVariableOrConstantDeclarationWithSourceExpressionForGeneration
    GALGAS_string var_sourceName = enumerator_3274.current_mValue (HERE).reader_lastPathComponent (SOURCE_FILE_AT_LINE (94)) ;
    //-------- @ifInstructionForGeneration
    const enumGalgasBool test_3 = var_sources.reader_hasKey (var_sourceName.reader_stringByDeletingPathExtension (SOURCE_FILE_AT_LINE (95)) COMMA_SOURCE_FILE_AT_LINE (95)).boolEnum () ;
    if (kBoolTrue == test_3) {
      { //-------- @typeMethodInstructionForGeneration
      GALGAS_string::class_method_generateFile (var_sourceDirectoryPath.operator_concat (enumerator_3274.current_mValue (HERE).reader_stringByDeletingLastPathComponent (SOURCE_FILE_AT_LINE (97)) COMMA_SOURCE_FILE_AT_LINE (97)), var_sourceName, var_fw.reader_textFileContentsAtPath (enumerator_3274.current_mValue (HERE), inCompiler COMMA_SOURCE_FILE_AT_LINE (99)), inCompiler COMMA_SOURCE_FILE_AT_LINE (96)) ;
      } //-------- @routineCallInstructionForGeneration
    }
