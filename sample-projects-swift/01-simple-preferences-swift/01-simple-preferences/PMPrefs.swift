//-----------------------------------------------------------------------------*
//                                                                             *
//                            File 'PMPrefs.swift'                             *
//                     Generated by version 0.0.1 [debug]                      *
//                       august 5th, 2014, at 15h28'11"                        *
//                                                                             *
//-----------------------------------------------------------------------------*

//--- START OF USER ZONE 1


//--- END OF USER ZONE 1

//----------------------------------------------------------------------------*

import Cocoa

//-----------------------------------------------------------------------------*
//    Protocol: myString                                                       *
//-----------------------------------------------------------------------------*

@objc(protocol_preference_2E_PMPrefs_2E_myString) protocol protocol_preference_2E_PMPrefs_2E_myString {
  func preference_2E_PMPrefs_2E_myString_didChange ()
}

//-----------------------------------------------------------------------------*
//    Protocol: mColor                                                         *
//-----------------------------------------------------------------------------*

@objc(protocol_preference_2E_PMPrefs_2E_mColor) protocol protocol_preference_2E_PMPrefs_2E_mColor {
  func preference_2E_PMPrefs_2E_mColor_didChange ()
}

//-----------------------------------------------------------------------------*
//    Protocol: mDate                                                          *
//-----------------------------------------------------------------------------*

@objc(protocol_preference_2E_PMPrefs_2E_mDate) protocol protocol_preference_2E_PMPrefs_2E_mDate {
  func preference_2E_PMPrefs_2E_mDate_didChange ()
}

//-----------------------------------------------------------------------------*
//    Protocol: mIntegerValue                                                  *
//-----------------------------------------------------------------------------*

@objc(protocol_preference_2E_PMPrefs_2E_mIntegerValue) protocol protocol_preference_2E_PMPrefs_2E_mIntegerValue {
  func preference_2E_PMPrefs_2E_mIntegerValue_didChange ()
}

//-----------------------------------------------------------------------------*

var g_PMPrefs : PMPrefs? = nil

//----------------------------------------------------------------------------*

@objc (PMPrefs) class PMPrefs : NSObject {
  //-----------------------------------------------------------------------------*
  //    Outlets                                                                  *
  //-----------------------------------------------------------------------------*

  @IBOutlet var myTextField : NSTextField?
  @IBOutlet var myOtherTextField : NSTextField?
  @IBOutlet var mColorWell : NSColorWell?
  @IBOutlet var mDatePicker : NSDatePicker?
  @IBOutlet var mInteger32TextField : NSTextField?

  //-----------------------------------------------------------------------------*
  //    Attribute: myString                                                      *
  //-----------------------------------------------------------------------------*

  var myString_observers = PMSet<protocol_preference_2E_PMPrefs_2E_myString> ()
  var myString : String = "hello" {
    didSet {
      if myString != oldValue {
        for anyObject in myString_observers.allObjects () {
          let object = anyObject as protocol_preference_2E_PMPrefs_2E_myString
          object.preference_2E_PMPrefs_2E_myString_didChange ()
        }
      }
    }
  }

  func addObserverOf_myString (inObserver : protocol_preference_2E_PMPrefs_2E_myString) {
    myString_observers.addObject (inObserver)
    inObserver.preference_2E_PMPrefs_2E_myString_didChange ()
  }
 
  func removeObserverOf_myString (inObserver : protocol_preference_2E_PMPrefs_2E_myString) {
    myString_observers.removeObject (inObserver)
    inObserver.preference_2E_PMPrefs_2E_myString_didChange ()
  }


  //-----------------------------------------------------------------------------*
  //    Attribute: mColor                                                        *
  //-----------------------------------------------------------------------------*

  var mColor_observers = PMSet<protocol_preference_2E_PMPrefs_2E_mColor> ()
  var mColor : NSColor = NSColor.yellowColor () {
    didSet {
      if mColor != oldValue {
        for anyObject in mColor_observers.allObjects () {
          let object = anyObject as protocol_preference_2E_PMPrefs_2E_mColor
          object.preference_2E_PMPrefs_2E_mColor_didChange ()
        }
      }
    }
  }

  func addObserverOf_mColor (inObserver : protocol_preference_2E_PMPrefs_2E_mColor) {
    mColor_observers.addObject (inObserver)
    inObserver.preference_2E_PMPrefs_2E_mColor_didChange ()
  }
 
  func removeObserverOf_mColor (inObserver : protocol_preference_2E_PMPrefs_2E_mColor) {
    mColor_observers.removeObject (inObserver)
    inObserver.preference_2E_PMPrefs_2E_mColor_didChange ()
  }


  //-----------------------------------------------------------------------------*
  //    Attribute: mDate                                                         *
  //-----------------------------------------------------------------------------*

  var mDate_observers = PMSet<protocol_preference_2E_PMPrefs_2E_mDate> ()
  var mDate : NSDate = NSDate () {
    didSet {
      if mDate != oldValue {
        for anyObject in mDate_observers.allObjects () {
          let object = anyObject as protocol_preference_2E_PMPrefs_2E_mDate
          object.preference_2E_PMPrefs_2E_mDate_didChange ()
        }
      }
    }
  }

  func addObserverOf_mDate (inObserver : protocol_preference_2E_PMPrefs_2E_mDate) {
    mDate_observers.addObject (inObserver)
    inObserver.preference_2E_PMPrefs_2E_mDate_didChange ()
  }
 
  func removeObserverOf_mDate (inObserver : protocol_preference_2E_PMPrefs_2E_mDate) {
    mDate_observers.removeObject (inObserver)
    inObserver.preference_2E_PMPrefs_2E_mDate_didChange ()
  }


  //-----------------------------------------------------------------------------*
  //    Attribute: mIntegerValue                                                 *
  //-----------------------------------------------------------------------------*

  var mIntegerValue_observers = PMSet<protocol_preference_2E_PMPrefs_2E_mIntegerValue> ()
  var mIntegerValue : Int64 = 123 {
    didSet {
      if mIntegerValue != oldValue {
        for anyObject in mIntegerValue_observers.allObjects () {
          let object = anyObject as protocol_preference_2E_PMPrefs_2E_mIntegerValue
          object.preference_2E_PMPrefs_2E_mIntegerValue_didChange ()
        }
      }
    }
  }

  func addObserverOf_mIntegerValue (inObserver : protocol_preference_2E_PMPrefs_2E_mIntegerValue) {
    mIntegerValue_observers.addObject (inObserver)
    inObserver.preference_2E_PMPrefs_2E_mIntegerValue_didChange ()
  }
 
  func removeObserverOf_mIntegerValue (inObserver : protocol_preference_2E_PMPrefs_2E_mIntegerValue) {
    mIntegerValue_observers.removeObject (inObserver)
    inObserver.preference_2E_PMPrefs_2E_mIntegerValue_didChange ()
  }


  //------------------------------------------------------- Arraies


  //------------------------------------------------------- Init
  override init () {
    super.init ()
    noteObjectAllocation (self) ;
    g_PMPrefs = self ;
     var ud = NSUserDefaults.standardUserDefaults ()
  //---
    var value : AnyObject?
    value = ud.objectForKey ("PMPrefs:myString")
    if value != nil {
      myString = value as NSString
    }
    value = ud.objectForKey ("PMPrefs:mColor")
    if value != nil {
      mColor = NSUnarchiver.unarchiveObjectWithData (value as NSData) as NSColor
    }
    value = ud.objectForKey ("PMPrefs:mDate")
    if value != nil {
      mDate = value as NSDate
    }
    value = ud.objectForKey ("PMPrefs:mIntegerValue")
    if value != nil {
      mIntegerValue = (value as NSNumber).longLongValue
    }
  //---
    NSNotificationCenter.defaultCenter ().addObserver (self,
     selector:"applicationWillTerminateAction:",
     name:NSApplicationWillTerminateNotification,
     object:nil
    )
  }

  //------------------------------------------------------- awakeFromNib
  override func awakeFromNib () {
  //--- check myTextField' outlet not nil
    if nil == myTextField {
      presentErrorWindow (__FILE__, __LINE__, "the 'myTextField' outlet is nil") ;
    }
    myTextField?.bind ("value",
      toObject:self,
      withKeyPath:"myString",
      options:nil
    )
  //--- check myOtherTextField' outlet not nil
    if nil == myOtherTextField {
      presentErrorWindow (__FILE__, __LINE__, "the 'myOtherTextField' outlet is nil") ;
    }
    myOtherTextField?.bind ("value",
      toObject:self,
      withKeyPath:"myString",
      options:[
        "NSContinuouslyUpdatesValue" : true
      ]
    )
  //--- check mColorWell' outlet not nil
    if nil == mColorWell {
      presentErrorWindow (__FILE__, __LINE__, "the 'mColorWell' outlet is nil") ;
    }
    mColorWell?.bind ("value",
      toObject:self,
      withKeyPath:"mColor",
      options:nil
    )
  //--- check mDatePicker' outlet not nil
    if nil == mDatePicker {
      presentErrorWindow (__FILE__, __LINE__, "the 'mDatePicker' outlet is nil") ;
    }
    mDatePicker?.bind ("value",
      toObject:self,
      withKeyPath:"mDate",
      options:nil
    )
  //--- check mInteger32TextField' outlet not nil
    if nil == mInteger32TextField {
      presentErrorWindow (__FILE__, __LINE__, "the 'mInteger32TextField' outlet is nil") ;
    }
    if (nil != mInteger32TextField) && (mInteger32TextField?.formatter == nil) {
      presentErrorWindow (__FILE__, __LINE__, "the 'mInteger32TextField' outlet has no formatter, it should have an NSNumberFormatter") ;
    }else if (nil != mInteger32TextField) && !((mInteger32TextField?.formatter?.isKindOfClass (NSNumberFormatter))!) {
      presentErrorWindow (__FILE__, __LINE__, "the 'mInteger32TextField outlet has a formatter that is not kind of NSNumberFormatter class") ;
    }
    mInteger32TextField?.bind ("value",
      toObject:self,
      withKeyPath:"mIntegerValue",
      options:nil
    )
  }
  
  //------------------------------------------------------- deinit
  deinit {
    noteObjectDeallocation (self) ;
  }
  
   //------------------------------------------------------- applicationWillTerminateAction
  func applicationWillTerminateAction (NSNotification) {
    var ud = NSUserDefaults.standardUserDefaults ()
    ud.setObject (myString, forKey:"PMPrefs:myString")
    ud.setObject (NSArchiver.archivedDataWithRootObject (mColor), forKey:"PMPrefs:mColor")
    ud.setObject (mDate, forKey:"PMPrefs:mDate")
    ud.setObject (NSNumber.numberWithLongLong (mIntegerValue), forKey:"PMPrefs:mIntegerValue")
  }

}

//----------------------------------------------------------------------------*

//--- START OF USER ZONE 2


//--- END OF USER ZONE 2



