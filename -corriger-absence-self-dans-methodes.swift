...
//-------------------- Chemin absolu vers projet galgas
  let projetGALGAS = "/Volumes/dev-svn/easy-bindings-dev/+easy-bindings.galgasProject"
...
//-------------------- Compiler
  ...
  while loop {
    print ("----------------------------------------------------------------------------------------------------------")
    let (s, status) = runHiddenCommand ("/usr/local/bin/galgas", ["--property-access-requires-self", "-q", "--no-color", "--max-errors=2", "--max-warnings=10000", projetGALGAS])
    if status == 0 {
